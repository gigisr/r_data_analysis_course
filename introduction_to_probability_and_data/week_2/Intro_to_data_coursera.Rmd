...
ggplot( data = sfo_feb_flights, aes( x = arr_delay ) ) +
  geom_histogram()
# Answer C - no flight is delayed more than 2 hours
```
...
```{r}
# Answer E - Both give us useful information
```
...
# Answer C - There is an overall positive association between distance and average speed
```
...
nycflights %>%
  summarise(del_dep_rate = sum(dep_type == "delayed" & arr_type == "on time") / n()) %>%
  arrange(desc(del_dep_rate))
```