...
ggplot(data = arbuthnot, aes(x = year, y = boys)) +
  geom_line() +
  geom_point()
# Overall it looks very similar to the first graph, however there are some differences
```
...
dim( present )[2]
# There are 3 columns, hence three variables
```
...
range(present$year)
# So the first year is 1940 and the most recent year is 2013
```
